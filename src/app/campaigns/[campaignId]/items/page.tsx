...
export default async function ItemsPage({ params, searchParams }: ItemsPageProps) {
  ...
  const [items, tags] = await Promise.all([
    listItems(campaignId, { q, tag, status, favorite, archived }, role),
  ...
  const entities = items.map((item) => ({
    ...
    excerpt: item.effect ?? item.description,
    statusBadges: (
      <>
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-4 sm:p-8">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Armas, artefatos e outros itens da campanha.</p>
        </div>
        <Button asChild>
          <Link href={`/campaigns/${campaignId}/items/new`}>
            <Plus className="size-4" /> Novo item
          </Link>
        </Button>
      </div>

      <WikiListToolbar tags={tags} statusOptions={CANON_STATUS_OPTIONS} searchPlaceholder="Buscar itens…" />

      {entities.length === 0 ? (
        <EmptyState
          message="Nenhum item criado ainda."
          action={
            <Button asChild>
              <Link href={`/campaigns/${campaignId}/items/new`}>
