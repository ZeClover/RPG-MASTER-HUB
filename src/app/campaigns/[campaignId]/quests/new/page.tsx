...
import { createQuestAction } from "@/modules/preparation/quests/actions";
...
export default async function NewQuestPage({ params }: NewQuestPageProps) {
  ...
  const action = createQuestAction.bind(null, campaignId);

  return (
    <div className="mx-auto w-full max-w-2xl p-4 sm:p-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold">Nova missão</h1>
        <p className="text-sm text-muted-foreground">Só o título é obrigatório.</p>
      </div>
      <QuestForm campaignId={campaignId} action={action} submitLabel="Criar missão" availableTags={tags} />
    </div>
  );
}
