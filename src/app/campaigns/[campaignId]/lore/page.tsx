...
interface LorePageListProps {
  params: Promise<{ campaignId: string }>;
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function LoreListPage({ params, searchParams }: LorePageListProps) {
  ...
  const [lorePages, tags] = await Promise.all([
    listLorePages(campaignId, { q, tag, status, favorite, archived }, role),
    listTagsForCampaign(campaignId),
  ]);

  const entities = lorePages.map((page) => ({
    id: page.id,
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-4 sm:p-8">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">A enciclopédia da campanha.</p>
        </div>
        <Button asChild>
      ...
      <WikiListToolbar tags={tags} statusOptions={CANON_STATUS_OPTIONS} searchPlaceholder="Buscar lore…" />

      {entities.length === 0 ? (
        <EmptyState
          message="Nenhuma página de lore criada ainda."
          action={
            <Button asChild>
              <Link href={`/campaigns/${campaignId}/lore/new`}>
