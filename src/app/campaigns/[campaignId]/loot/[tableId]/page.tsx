...
interface LootTableDetailPageProps {
  params: Promise<{ campaignId: string; tableId: string }>;
}

export async function generateMetadata({ params }: LootTableDetailPageProps): Promise<Metadata> {
  const { campaignId, tableId } = await params;
  const user = await requireUser();
  const table = await getRollTableForUser(user.id, campaignId, "LOOT", tableId).catch(() => null);
  return { title: table?.name ?? "Tabela de loot" };
}

export default async function LootTableDetailPage({ params }: LootTableDetailPageProps) {
  const { campaignId, tableId } = await params;
  const user = await requireUser();
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 p-4 sm:p-8">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button asChild variant="outline" size="sm">
          <Link href={`/campaigns/${campaignId}/loot/${tableId}/edit`}>
