...
## 24. Fase 13 — Construtor de Sistema

O pedido do dono do produto para esta fase, verbatim (traduzido): "Construtor de Sistema — Atributos, Recursos, Perícias, Fórmulas de rolagem, Condições da campanha, Modelo de ficha criado pelo mestre, Campos secretos na ficha, Botões de rolagem na ficha." O hub sempre tratou "sistema de jogo" como texto livre espalhado pelo produto — `MonsterAttribute.key`/`value` (Fase 6, seção 17.1), `Power.cost` como string solta (seção 17.2), o Dice Roller do Modo Sessão aceitando qualquer notação `NdM` sem saber o que ela representa (Fase 3). Esta fase é a primeira vez que o hub modela o *sistema de regras* da campanha como dado de verdade — atributos, recursos, perícias e fórmulas nomeados, com uma `key` estável que outras partes do produto podem referenciar — em vez de deixar cada ferramenta reinventar seu próprio texto livre.

**Fronteira de escopo, explícita desde o pedido original**: a próxima fase (Fase 14, "Personagens e Arsenal Conectado") é quem liga essas definições a `Character` de verdade — valores preenchidos por jogador, recursos com atual/máximo rastreados ao vivo, botões de rolagem usáveis durante uma sessão real e logados no Session Log. Esta fase entrega só a camada de DEFINIÇÃO: o mestre define o que existe no sistema da campanha e organiza um layout de ficha — nada aqui toca `Character`. O único lugar onde uma rolagem de verdade acontece é a ferramenta "Testar fórmula" (seção 24.2), e ela é deliberadamente uma prévia de builder, não uma rolagem de mesa.

### 24.1 Seis modelos de definição, um template implícito por campanha (não vários)

`AttributeDef`, `ResourceDef`, `SkillDef`, `ConditionDef`, `RollFormulaDef` e `SheetSection` (`prisma/schema.prisma`) são todos filhos diretos de `Campaign`, sem tabela de junção entre si — o mesmo desenho "flat, `@@index([campaignId, order])`, `onDelete: Cascade`" já usado por `RollTable`/`NarrativeClock` (Fases 5/6). `AttributeDef`/`ResourceDef`/`SkillDef` carregam uma `key` (`@@unique([campaignId, key])`) gerada a partir do `name` com `slugifyTagName` (o mesmo utilitário de `createTagAction`, Fase 1) — mas adaptado: `Tag` dedup por slug (nome duplicado = mesma tag), enquanto aqui cada definição precisa de uma `key` própria, então `generateUniqueDefKey` (`modules/gametools/system/key.ts`) acrescenta um sufixo numérico (`forca`, `forca-2`, …) em vez de devolver a linha existente. `SkillDef.relatedAttributeId` é opcional e só para organização/exibição (`onDelete: SetNull` — apagar o atributo relacionado deixa a perícia "sem atributo relacionado", nunca quebra a perícia); a fórmula de rolagem de uma perícia não é um campo dela, é um `RollFormulaDef` à parte, que pode referenciar a perícia por token como referencia qualquer atributo.

`SheetSection` modela deliberadamente **um único** layout de ficha por campanha — uma lista ordenada de seções (`kind: ATTRIBUTES | RESOURCES | SKILLS | CONDITIONS | FORMULAS | CUSTOM_TEXT`), cada uma um marcador de "aqui entra a lista de X" (sem duplicar dado — só `CUSTOM_TEXT` carrega conteúdo próprio, em `customText`). O pedido do dono do produto falava só em "modelo de ficha criado pelo mestre", no singular; um sistema de múltiplos templates por campanha (aluno vs. professor vs. monstro, o exemplo mais óbvio) foi triado explicitamente para uma fase futura no próprio brief desta fase — implementar isso agora seria resolver um problema que não foi pedido, à custa de mais uma tabela (`SheetTemplate`) e mais uma FK em todo lugar que hoje assume "a ficha da campanha". "Criar seções padrão" (`createDefaultSheetSectionsAction`, `modules/gametools/system/sheet-actions.ts`) é a única affordance de conveniência: um clique cria uma `SheetSection` por `kind` (exceto `CUSTOM_TEXT`, que não tem "padrão" — é sempre texto livre) nos `kind`s que ainda faltam, sem sobrescrever o que já existe.

Nenhum dos 6 modelos tem reordenação por arrastar ou botões subir/descer — `order` é atribuído só na criação (`último + 1`), o mesmo padrão já estabelecido por `RollTableEntry`/`MonsterAttribute`/`ChecklistItem` (Fases 2/5/6) em todo o código-base: nenhuma lista com `order` deste produto tem UI de reordenação, então inventar uma aqui, só para o Construtor de Sistema, seria uma UI mais sofisticada que qualquer outra tela já shippada — fora do que este trabalho deveria decidir sozinho.

### 24.2 Tokens `{key}` e um avaliador de fórmulas sem `eval()`

Uma fórmula (`RollFormulaDef.formula`, ex. `"1d20 + {forca} + {treinamento}"`) é texto livre com tokens `{key}` embutidos — nunca uma expressão arbitrária, nunca avaliada com `eval()`/`Function()`/qualquer execução de código dinâmico. `src/lib/formula.ts` é um parser puro (mesmo espírito "sem I/O" de `src/lib/dice.ts`, que ele usa por baixo e nunca duplica): `extractFormulaTokens` acha todo `{key}` com regex; `substituteFormulaTokens` troca cada um pelo valor numérico correspondente (0 quando ausente); `validateFormulaTokens` devolve os tokens que não existem no conjunto de `key`s conhecidas da campanha; `evaluateFormula` substitui, localiza a ÚNICA rolagem de dados dentro do resultado (reaproveitando o padrão `NdM` de `dice.ts`, buscado dentro da string maior em vez de ancorado a ela inteira), rola via `rollDice` de verdade (nunca reimplementa RNG) e soma todo termo `[+-] número` restante como modificador plano — v1 propositalmente restrita a "uma rolagem + modificadores somados", cobrindo todo exemplo que o dono do produto deu; zero ou mais de uma rolagem de dados são erros claros, não um resultado parcial silencioso.

A validação de tokens desconhecidos acontece em dois pontos, com efeitos diferentes por design: **ao salvar** (`createRollFormulaAction`/`updateRollFormulaAction`, `modules/gametools/system/formula-actions.ts`) uma fórmula que referencia uma `key` inexistente é rejeitada com erro claro listando os tokens — não há como criar uma fórmula quebrada do zero. Mas excluir um `AttributeDef`/`SkillDef` cujo token uma fórmula já existente referencia **é permitido**, não bloqueado — a alternativa (impedir a exclusão até toda fórmula dependente ser corrigida/apagada) transformaria uma limpeza rotineira do mestre num quebra-cabeça de dependências. Em vez disso: o diálogo de confirmação de exclusão lista, por nome, toda fórmula afetada antes do clique final (mesmo padrão de diálogo de confirmação já usado pelo resto do produto, `ConfirmDialog`/`ArchiveCampaignButton`/`DeleteCharacterButton`), e a lista de Fórmulas sinaliza permanentemente qualquer fórmula com token desconhecido com um badge "Token desconhecido: {x}" — o mestre é avisado nos dois momentos (antes de agir, e toda vez que revisita a lista depois), sem o produto fingir que a fórmula quebrada não existe nem travar o fluxo de exclusão para resolver algo que pode ser só um refactor de nomes em andamento.

A ferramenta "Testar fórmula" (`FormulaTestDialog`, `components/system/formula-test-dialog.tsx`) roda `evaluateFormula` inteiramente no cliente, com valores que o mestre digita manualmente para cada token (pré-preenchidos com o `defaultValue`/`defaultBonus` da definição) — o mesmo raciocínio já aceito de rolagem client-side em `dice-roller.tsx` (Modo Sessão): é RNG de jogo, não um limite de segurança a proteger no servidor. O diálogo mostra uma nota explícita ("Rolagem de teste — não é salva na sessão") e deliberadamente não escreve em `SessionLogEntry` nem em nenhum outro estado — é uma prévia de builder time, não uma ação de mesa real; ligar isso a uma rolagem de verdade, registrada no log, é trabalho da Fase 14, quando existir um `Character` de onde puxar valores reais em vez de números digitados à mão.

### 24.3 Leitura PLAYER, escrita CO_GM — e por que isso difere do gate mais estrito de `RollTable`

`RollTable` (Fase 6, seção 17.3) exige `CO_GM` até para **ler** — é ferramenta de bastidor do mestre (uma tabela de encontros aleatórios não é algo que o jogador deveria poder espiar antes de rolar). O Construtor de Sistema inverte essa lógica de propósito: as 6 listagens (`listAttributes`/`listResources`/`listSkills`/`listConditions`/`listRollFormulas`/`listSheetSections`, `modules/gametools/system/queries.ts`) exigem só `requireCampaignAccess(userId, campaignId)` — o mínimo padrão, `PLAYER` — porque estas não são notas de preparação do mestre, são **as regras do próprio sistema de jogo**: um jogador precisa saber o que "Força" significa nesta campanha, que perícias existem, que condições podem ser aplicadas a ele, exatamente como precisaria numa ficha de papel de verdade. Esconder isso do jogador quebraria o propósito da própria feature. Mutações (`create*Action`/`update*Action`/`delete*Action`, `modules/gametools/system/{actions,formula-actions,sheet-actions}.ts`) continuam exigindo `CO_GM`, o mesmo corte já usado por NPCs/Monstros/Itens/Poderes (Fases 1/6): qualquer um vê a lista, só quem mestra edita.

O único ajuste fino dentro desse "leitura aberta" é `gmOnly`, um booleano por linha em `AttributeDef`/`ResourceDef`/`SkillDef`/`SheetSection` (não em `ConditionDef`/`RollFormulaDef` — ver o raciocínio por modelo nos comentários do schema) — o "campos secretos na ficha" do pedido original. `filterGmOnlyForRole` (`queries.ts`) remove a linha inteira da resposta para `PLAYER`, nunca só um campo dela (diferente de `stripGmFields`, Fase 9, que zera campos específicos mas mantém a entidade visível) — porque aqui a linha inteira *é* o segredo: um atributo GM-only (ex. "Sanidade Real" que só o mestre acompanha) não deveria nem aparecer como existindo para o jogador, muito menos ter seu nome/descrição visíveis com o valor escondido.

### 24.4 O que esta fase deliberadamente NÃO faz: `Character` fica intocado

Vale repetir, porque a tentação de ligar seria real e tornaria a feature "parecer mais real": nenhum código desta fase lê, escreve ou referencia `Character` de nenhum jeito. Nenhuma migração adiciona campo a `Character`; nenhuma query de `modules/gametools/system` faz join com `characters`; a ferramenta "Testar fórmula" (seção 24.2) pede valores digitados à mão em vez de puxar de uma ficha de jogador justamente porque não existe, nesta fase, uma ficha de jogador preenchida com valores de sistema para puxar. Isso é decisão do dono do produto, não uma limitação técnica descoberta durante o trabalho — a Fase 14 ("Personagens e Arsenal Conectado") é quem, por design, herda o trabalho de: adicionar valores por atributo/recurso a cada `Character`, rastrear atual/máximo de recurso ao vivo, tornar os botões de rolagem desta fase realmente usáveis durante uma sessão (gravando em `SessionLogEntry`), e aplicar/remover `ConditionDef` a um personagem específico. Até lá, o Construtor de Sistema é puramente um editor do "livro de regras" da campanha — poderoso para o mestre organizar o sistema, sem nenhuma ficha de personagem de verdade do outro lado.
...
| **13 — Construtor de Sistema** ✅ | Atributos, Recursos, Perícias, Condições, Fórmulas de rolagem (avaliador seguro sem `eval()`) e um Modelo de Ficha por campanha (`SheetSection`) definidos pelo mestre, com campos secretos (`gmOnly`) e uma ferramenta "Testar fórmula" — leitura aberta a `PLAYER`, escrita `CO_GM`; deliberadamente sem tocar `Character` (fica para a Fase 14) |

O roadmap original terminava na Fase 9; a Fase 10 é a primeira fase pós-roadmap original, motivada por pedido direto do dono do produto (seção 21), a Fase 11 é a segunda — desta vez não pedindo módulo novo nenhum, e sim que as dez fases anteriores fechassem suas próprias lacunas (seção 22) — a Fase 12 é a terceira, mudando de eixo outra vez: não é sobre conteúdo de campanha nem sobre arrumar a casa, é sobre o que protege os dados e o acesso de quem já usa o produto, e sobre garantir (via teste automatizado, pela primeira vez) que as fases futuras não quebrem as onze anteriores sem ninguém notar (seção 23) — e a Fase 13 é a quarta, voltando a adicionar módulo novo (o item "Sistema" da navegação, órfão desde a Fase 9 como placeholder "em breve"), mas deliberadamente só a camada de definição de regras, sem tocar `Character`: a Fase 14 é quem liga este sistema a personagens de verdade (seção 24). Cada fase começou lendo o código existente, reaproveitando `modules/core` e os componentes de `components/ui`, sem reescrever o que já funciona — o mesmo vale para qualquer trabalho futuro sobre este projeto.
